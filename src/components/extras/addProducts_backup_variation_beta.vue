  ...
  <div>
    <div class="adding-box-wrap">
      ...
      <div class="container padding_mob" v-else>
        ...
        <div class="adding-box-1-cnt wrapIn col-sm-12 pb_category">
          <div class="sectionTitle">
            <div class="d-inline-flex align-center">
              <span class="squareBadge mr-3">4</span>Product Variations
            </div>
            <button
              @click="toggleDiv('showProductVariations')"
              type="button"
              class="md-button md-theme-default toggleboxBtn"
            >
              <span class="material-icons" v-if="showProductVariations"
                >keyboard_arrow_up</span
              >
              <span class="material-icons" v-else>keyboard_arrow_down</span>
            </button>
          </div>
          <div v-if="showProductVariations">
            <div class="md-layout md-gutter">
              <div class="pillWrap md-layout-item md-size-100 mt-4">
                <div class="radio-pill">
                  <md-radio
                    class="item"
                    :value="false"
                    v-model.trim="isVariableProduct"
                    >Simple Product</md-radio
                  >
                </div>
                <div class="radio-pill">
                  <md-radio
                    ...
                    :value="true"
                    v-model.trim="isVariableProduct"
                    >Variable Product</md-radio
                  >
                </div>
              </div>

              <div
                class="md-layout-item md-size-100 mt-4"
              ...
              >
                <a
                  ...
                  "
                >
                  <span class="material-icons">add</span>Add Variation
                </a>
                <div
                  class="
                    ...
                    variationList
                  "
                  v-if="this.form.variations.length > 0"
                >
                  <div
                    class="md-layout-item md-layout md-size-50 category"
                    v-for="(variation, varId) in form.variations"
                    :key="varId"
                  >
                    <p class="md-layout-item md-size-100 sectionTitle">
                      {{ variation.name }}
                    </p>
                    <div
                      class="md-layout-item md-size-45 md-layout"
                      v-for="(variationOption, opId) in variation.options"
                      :key="opId"
                    >
                      <div class="variationOption md-layout">
                        <div class="md-layout-item md-size-100">
                          <img :src="variationOption.files_urls[0]" />
                        </div>
                        <div
                          class="md-layout-item md-size-100 variationOptionName"
                        >
                          {{ variationOption.name }}
                        </div>

                        <div
                          class="md-layout-item md-size-50 variationDetailsLeft"
                        >
                          Qty: {{ variationOption.qty }}
                        </div>
                        <div class="md-layout-item md-size-50">
                          <div
                            class="dot"
                            :style="{
                              'background-color': variationOption.value,
                            }"
                          >
                            <div class="letter">
                              {{
                                variation.name === "size"
                                  ? variationOption.value.charAt(0)
                                  : ""
                              }}
                            </div>
                          </div>
                        ...
                        >
                          ...
                          <md-button
                            class="
                              md-accent md-layout-item md-size-45
                              edit_button
                            "
                            @click="editVariation(varId, opId)"
                          >
                          ...
                          <md-button
                            ...
                            "
                            @click="deleteVariationOption(varId, opId)"
    ...
    <md-dialog :md-active.sync="showDialog">
      <md-dialog-title class="title_modal_card mob_card_pl"
        >Add New Variation</md-dialog-title
      >
      <md-button class="md-primary close_btn" @click="showDialog = false">
        <i class="material-icons"> close </i>
      </md-button>
      <md-dialog-content class="md-layout mt-4 modal_form">
        <input-component
          label="Attribute *"
          custom="true"
          :size="50"
          :error="variationFormSubmitted && $v.variationForm.variation.$invalid"
        >
          <v-select
        ...
        <input-component
          ...
          :size="50"
          :error="variationFormSubmitted && $v.variationForm.name.$invalid"
        >
        </input-component>
        <input-component
          v-if="variationForm.variation === 'color'"
          label="Choose Color *"
          v-model="variationForm.value"
          type="color"
          :size="50"
          :error="variationFormSubmitted && $v.variationForm.value.$invalid"
        >
        </input-component>
        <input-component
          v-else-if="variationForm.variation === 'size'"
          label="Choose Size *"
          :custom="true"
          :size="50"
          :error="variationFormSubmitted && $v.variationForm.value.$invalid"
        ...
        <input-component
          ...
          v-model="variationForm.qty"
          type="number"
          :size="50"
          :error="variationFormSubmitted && $v.variationForm.qty.$invalid"
        >
        </input-component>
        <input-component
          custom="true"
          :size="100"
          :error="
            variationFormSubmitted && this.$v.variationForm.files.$invalid
          "
          errorMessage="Kindly add at least one Image."
        >
          <div class="variationFileUpload uploadWrap">
            <div class="add-meadia-bx">
              <div>
                <div v-if="isCropped_variation">
                  <Cropper
                    :restrictions="pixelsRestriction"
                    :src="img_variation"
                    @change="change_variation"
                  ></Cropper>
                  <div class="save-image">
                    <button
                      class="md-button md-raised md-accent md-theme-default"
                      @click="saveImage_variation"
                    >
                      Save
                    </button>
                    <button
                      class="md-button md-raised md-accent md-theme-primary"
                      @click="CancelCrop_variation"
                    >
                      Cancel
                    </button>
                  </div>
                ...
                <div v-if="before_crop_variation" class="mb-3">
                  <div class="img-selected-wrapper">
                    <div
                      class="selected-images"
                      v-for="(variation_img, index) in variationForm.files_urls"
                      :key="index"
                    >
                      <img :src="variation_img" />

                      <span
                        class="material-icons removeIco"
                        @click="delete_image_variation(index)"
                        >close</span
                      >
                    </div>
                  </div>
              ...
              <div class="uploader-box">
                <div
                  @click="chooseFile_variation"
                  v-if="ShowChooseFile_variation"
                >
                  <div class="choose-file-btn md-button md-theme-default">
                    <input
                      type="file"
                      ref="file_variation"
                      style="display: none"
                      v-on:change="chooseImage_variation($event)"
                    />
                    Add Files
                    <i class="material-icons">add</i>
                  </div>
...
export default {
  ...
  validations() {
    ...
    const variationFormFields = {
      ...
      files_urls: { required: false },
    };

    if (this.showDialog) {
      variationFormFields.variation = { required };
      variationFormFields.name = { required };
      variationFormFields.value = { required };
      variationFormFields.qty = { required };
      variationFormFields.files = { required };
      variationFormFields.files_urls = { required };
    }
    return {
    ...
    return {
      ...
      breadcrumbs: [
        ...
        },
        {
          ...
          link:
            "/dashboard/vendor/add?market_type=" +
  ...
  methods: {
    ...
    clearVariationForm() {
      this.variationFormSubmitted = false;
      this.variationForm = {
        variation: "",
        name: "",
        value: "",
    ...
    deleteVariationOption(variationId, optionId) {
      alert(
        JSON.stringify(this.form.variations[variationId].options[optionId])
      );
      this.form.variations[variationId].options.splice(optionId, 1);
    },
    editVariation(variationId, optionId) {
      alert(
        JSON.stringify(this.form.variations[variationId].options[optionId])
      );
      this.variationForm = {
        variation: this.form.variations[variationId].name,
        name: this.form.variations[variationId].options[optionId].name,
        value: this.form.variations[variationId].options[optionId].value,
        qty: this.form.variations[variationId].options[optionId].qty,
        files: this.form.variations[variationId].options[optionId].files,
        files_urls:
          this.form.variations[variationId].options[optionId].files_urls,
      };
    ...
    async addNewVariation() {
      ...
      this.$v.variationForm.$touch();
      this.variationFormSubmitted = true;

      if (this.$v.variationForm.$invalid) {
        return;
      }
      const AttributeIndexInVariations = this.form.variations.findIndex(
        (value, index) => value.name === this.variationForm.variation
      );
      this.form.variations[AttributeIndexInVariations].options.push({
        name: this.variationForm.name,
        value: this.variationForm.value,
        qty: this.variationForm.qty,
        files: this.variationForm.files,
        files_urls: this.variationForm.files_urls,
      });
    ...
    async uploadVariationFiles() {
      this.form.variations = await this.form.variations.filter(
        (variation) => variation.options.length > 0
      );

      for (let var_id = 0; var_id < this.form.variations.length; var_id++) {
        for (
          let var_op_id = 0;
          var_op_id < this.form.variations[var_id].options.length;
          var_op_id++
        ) {
          ...
          const resp = await this.getFileUrls_variation(
            this,
            this.form.variations[var_id].options[var_op_id].files,
            files_cloud_urls
          );
