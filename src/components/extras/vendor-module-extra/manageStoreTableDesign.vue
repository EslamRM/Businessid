    ...
    <div class="col-sm-7 col-md-6 col-xl-12 fl wrapper_order_table">
      <div class="col-sm-12 pl-0 pr-0 fl">
        ...
        <div class="md-layout md-size-100 md-alignment-top-space-between caps">
          <div
            class="md-layout-item md-size-25 md-xsmall-size-100 md-small-size-100 box_date"
          >
            <input
              type="text"
          ...
          <div
            class="md-layout-item md-layout filters md-size-75 md-small-size-100 md-xsmall-size-100"
          >
            <div
      ...
      >
        <md-table>
          <md-table-row>
            ...
            <md-table-head v-if="products[0].type != 'auto'"
              ><h3>Unit</h3></md-table-head
            >
          ...
          <md-table-row v-for="(product, id) in products" :key="id">
            <md-table-cell md-numeric> {{ product.item_id }} </md-table-cell>
            <md-table-cell class="table_padding">
              <img
                :alt="product.name"
                :src="product.images.length > 0 ? product.images[0].file : ''"
              />
            </md-table-cell>
            ...
            <md-table-cell class="table_padding">
              {{ product.brand }}</md-table-cell
            >
            <md-table-cell
              class="table_padding"
              v-if="products[0].type != 'auto'"
            >
              {{ product.unit }}
            </md-table-cell>
            <!-- <md-table-cell class="table_padding">
            ...
            <md-table-cell class="table_padding">
              ...
              <span v-if="product.quantity <= 0" class="out_of_stock"
                >Out Of Stock</span
              >
            </md-table-cell>
            ...
            <md-table-cell class="table_padding">
              {{
                currency(product.offerprice, product.currency) || "N/A"
              }}</md-table-cell
            >
            <md-table-cell class="table_padding">
              <span
                ...
                @click="changeStatus(product.status, product.item_id)"
              >
                {{ product.status === "1" ? "Active" : "In-Active" }}
              </span>
              <md-tooltip md-direction="top"
                >Click here to make it
                {{ product.status === "2" ? "Active" : "In-Active" }}
              </md-tooltip>
            </md-table-cell>
            <md-table-cell class="display_flex">
              <i
                class="material-icons action_icon mr-3"
                style="color: #bbb; cursor: not-allowed"
                >edit</i
              >
              <i
                class="material-icons action_icon"
        ...
        <md-table>
          ...
          <transition name="fade">
            <md-table-row v-if="noMoreInvoices">
              <md-table-cell colspan="10" md-numeric
                ><center>
                  <h1 class="no-orders-found">No more products.</h1>
                </center></md-table-cell
              >
            </md-table-row>
          </transition>
...
export default {
  ...
  // created () {
  //   this.loadPendingOrders("firstLoad");
  //   this.whichType()
  // },
  watch: {
    "$route.params": {
      ...
      try {
        ...
        await this.$swal.fire(
          this.$helpers.errorPopup(
            `Error making it ${status === "1" ? "Active" : "In-Active"
            }. Please try again.`
          )
        );
        console.log(error);
