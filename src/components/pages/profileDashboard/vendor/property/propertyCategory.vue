<template>
  <div>
    ...
    <div class="row_grid_mob">
      <div class="grd-tp-bx-1 list-itm col-sm-12 col-md-12 col-xl-6 p-0">
        <div v-for="(item, index) in secondLevel" :key="index">
          <router-link
            ...
            :value="item.name"
            class="lnk-tp-lst-1 list-1"
          >
            <div class="lnk-tp-lst-1-bx list-2">
              <span class="lnk-txt-1">{{ item.name }}</span>
            </div>
          </router-link>
        </div>
      </div>
...
export default {
  ...
  beforeMount() {
    ...
    this.loadMore();
    this.getSecondLevelSale(this.$route.query.purpose_id);
  },
  methods: {
    ...
    category_type(event, id) {
      ...
      if (this.filters.property_category == null) {
        ...
        this.$router.push({
          path: "/dashboard/vendor/add-new-property/property-type",
          query: { category: event, category_id: id }
        });
      }
    },
