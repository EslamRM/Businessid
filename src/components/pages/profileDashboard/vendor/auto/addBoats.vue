            ...
            <div v-if="form.category">
              ...
              >
                ...
                <input-component
                  ...
                  :error="checkValidation('drive_system')"
                ></input-component>
              </div>
            </div>
          </transition></div
...
export default {
  name: "addCar",
  // uploadFiles is defined in this Mixin (getFileUrls)
  mixins: [utils],
  components: {
    inputComponent,
    breadcrumbs,
  ...
  data () {
    return {
      ...
      breadcrumbs: [
        ...
        {
          title: this.demandOrSupplyPrefix() + " Boats",
          link:
            "/dashboard/vendor/addBoat?market_type=" +
            this.$route.query.market_type
        }
      ],
      extraFeatures: [
  ...
  created () {
    ...
    this.getAutoTypes("api_auto_types", { parentid: "2" }, "sub_categories");
    this.getAutoTypes(
  ...
  methods: {
    ...
    async submitForm () {
      ...
      this.form.tags = await this.tags.map(tag =>
        tag.text.replaceAll(/[^a-zA-Z ]/g, "")
      );
