    ...
    >
      <div class="md-layout-item">
        <div class="frm-hdr-1-txt txt-algn-lft">
          {{ demandOrSupplyPrefix() }} Motorcycles
        </div>
      </div>
...
export default {
  ...
  data () {
    return {
      ...
      breadcrumbs: [
        ...
        {
          title: this.demandOrSupplyPrefix() + " Motorcycles",
          link: "/dashboard/vendor/addMotorcycles?market_type=" + this.$route.query.market_type,
        }
      ],
      extraFeatures: [
        {
  ...
  created () {
    ...
    this.getAutoTypes("api_auto_types", { parentid: "4" }, "sub_categories");
    this.getAutoTypes(
  ...
  methods: {
    ...
    async submitForm () {
      ...
      this.form.tags = await this.tags.map(tag => tag.text.replaceAll(/[^a-zA-Z ]/g, ""));
