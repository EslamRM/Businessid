...
<template>
  <div>
    <div class="md-layout md-alignment-center-space-between page-title">
      <div class="md-layout-item">
        <div class="frm-hdr-1-txt txt-algn-lft">
          {{ demandOrSupplyPrefix() }} Accessories & Spare Parts
        </div>
      </div>
        ...
        <transition name="expand">
          <div v-if="autoProductInformation">
            <div class="md-layout md-gutter mt-4 md-alignment-center-center">
              ...
              >
                <vue-editor v-model="form.information"></vue-editor>
              </input-component>
            </div>
          </div>
        </transition>
...
export default {
  ...
  data () {
    return {
      ...
      breadcrumbs: [
        ...
        {
          title: this.demandOrSupplyPrefix() + " Accessories",
          link:
            "/dashboard/vendor/addAccessoriesAndSpareParts?market_type=" +
            this.$route.query.market_type
        }
      ],
      extraFeatures: [
  ...
  created () {
    ...
    this.getAutoTypes("api_auto_types", { parentid: "985" }, "sub_categories");
    this.getAutoTypes(
  ...
  methods: {
    ...
    //     // await this.RealArray.forEach(async file => {
    //     //   // Prepare Form Data
    //     //   let formData = await new FormData();
    //     //   await formData.append("files[]", file);
    //     //   // Now Send it
    ...
    async submitForm () {
      ...
      this.form.tags = await this.form.tags.map(tag =>
        tag.replaceAll(/[^a-zA-Z ]/g, "")
      );
      this.form.color = await this.form.colors;
      ...
      if (await !response.data.success) {
        ...
        await this.$swal.fire(
          this.$helpers.errorPopup(
            "Error Adding your Auto." + response.data.message
          )
        );
        return;
      } else {
