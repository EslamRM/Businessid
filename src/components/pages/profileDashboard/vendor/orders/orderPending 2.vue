<template>
  <div>
    <div class="col-sm-7 col-md-7 col-xl-12 fl wrapper_order_table">
      ...
      >
        <md-table>
          ...
          <md-table-row v-for="(order, index) in orders" :key="index" class="bg_row">
            ...
            >
            <!-- To revert back change uncomment code below -->
            <!-- <md-table-cell
              >{{ getTotalQuantity(index) }}
              {{
                order.items.length > 1 ? `multiple` : `${order.items[0].unit}`
              }}</md-table-cell
            > -->
            <md-table-cell>
              {{
    ...
    async loadPendingOrders (type) {
      ...
      if (!success) {
        if (message && message === "no orders found") {
          type === "refresh" ? this.noInvoicesFound = true : this.noMoreInvoices = true
        }
