...
export default {
  ...
  data() {
    return {
      ...
      propertyPurposes: [],
      property_purpose: this.$route.query.purpose,
      propertyCategories: [],
      property_category: this.$route.query.category,
      propertyTypes: [],
  ...
  methods: {
    ...
    async propertyCategorySelected(data, iteration) {
      ...
      if( data.id==8 || data.id==4 ){
        this.form.attached_kitchen='';
        this.form.bathrooms='';
        this.form.bedrooms='';
        this.isNonCommercial = false;
      }
      else this.isNonCommercial = true;
      const response = this.fetchAndAssign(
        "property-types",
        { parentid: data.id },
        "propertyTypes"
    ...
    // },
    async submitForm() {
      this.form.latitude = 25.204849;
      this.form.longitude = 55.270782;
      this.loading = true;
      this.submitted = true;
      this.$v.$touch();
      if (this.$v.$invalid) {
